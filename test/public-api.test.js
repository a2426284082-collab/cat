...
test('public API filters on server, exposes only allowed fields, and protects images', async () => {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    if (target.includes('/records')) {
      return Response.json({ code: 0, data: { has_more: false, items: [
        { fields: { '猫咪ID': 'CAT001', '状态': '在售', '品种': '英短', '毛色/花色': '金渐层', '性别': '公', '年龄': 4, '价格': 3800, '图片': [{ file_token: 'public_file' }], '内部备注': '不要公开' } },
        { fields: { '猫咪ID': 'CAT002', '状态': '下架', '品种': '布偶', '图片': [{ file_token: 'private_file' }] } },
      ] } });
    }
  ...
  try {
    ...
    assert.deepEqual(body.data[0], { id: 'CAT001', breed: '英短', color: '金渐层', gender: '公', age: '4', price: 3800, image: '/api/public-images/public_file' });
