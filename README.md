...
在**同一个飞书多维表格**新建**附件类型**的列，准确命名为 `视频`。一只猫的一格中可以上传多个 MP4 或 WebM 视频（建议 MP4、H.264/AAC），原有 `图片` 列继续充当封面。有视频附件时，访客点击照片上的播放按钮可在弹窗中选择视频；无视频的照片保持原样。更新通常需要数分钟。不再需要 `视频链接` 列。
