...
网站导航和首页均提供“新人教程”入口，访问路径为 `/guide/`。教程是独立静态文件 `public/guide/index.html`，不依赖飞书或网站接口。以后更新教程时，保持文件名与路径不变，直接替换这一份 HTML 后重新部署即可，主站代码无须修改。
...
可按价格（元）和月龄（月）填写最低、最高范围，边界值包含在结果中；资料缺少价格或月龄的猫咪在使用对应筛选时不会出现在结果中。可随时清除筛选条件。

在飞书现有 `图片` 附件列为一只猫上传多张照片，卡片下方会显示可切换的缩略图。点击右下角的放大按钮可查看大图并切换；有视频时，点击主照片仍会打开视频。无须另建图片列。
...
网页 HTML 声明 `noindex`，公开数据与附件响应也设置 `X-Robots-Tag`，要求遵守该协议的搜索引擎不要收录本站。不要用 `robots.txt` 阻止爬虫读取首页，否则它可能看不到 `noindex`。不收录不等于私密，知道网址的人仍可以访问。若旧网址已出现在搜索结果，需要等待爬虫重新抓取，或到对应搜索站长平台申请移除。
